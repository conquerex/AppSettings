...
public class RestAdapter {
    ...
    public synchronized static ISeoulOpenData getInstance(){
        if(service == null){
            ...
            // okhttp 설정
            // SSL을 통한 서버 연결인 경우에 인증서가 없으면 통신 자체가 되지 않는다
            // 이런 경우 회피하기 위해 인증서를 무시하는 세팅을 해줄 수 있다
            client = configureClient(new OkHttpClient().newBuilder())   // 인증서를 무시할 수 있는 세팅
                    .connectTimeout (CONNECT_TIMEOUT, TimeUnit.SECONDS)
                    .writeTimeout   (WRITE_TIMEOUT, TimeUnit.SECONDS)
                    .readTimeout    (READ_TIMEOUT, TimeUnit.SECONDS)
                    .cookieJar(new JavaNetCookieJar(manager))           // 쿠키를 저장하기 위한 공간 설정 - urlconnection 라이브러리가 필요하다
                    .addInterceptor(interceptor)                        // 로그를 출력
                    .build();
            ...
            service = new Retrofit.Builder()
                    ...
                    .build().create(ISeoulOpenData.class);
        }
        return service;
    }
