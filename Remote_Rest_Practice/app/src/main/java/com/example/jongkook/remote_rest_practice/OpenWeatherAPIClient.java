...
public class OpenWeatherAPIClient {
    ...
    public Weather getWeather(int lat,int lon){
        ...
        try {
            ...
            // HTTP 호출을 하기 위해서는 HttpURLConnection이라는 클래스를 사용
            HttpURLConnection urlConnection = (HttpURLConnection) url.openConnection();
//            urlConnection.setConnectTimeout(CONNECTION_TIMEOUT);
//            urlConnection.setReadTimeout(DATARETRIEVAL_TIMEOUT);

            InputStream in = new BufferedInputStream(urlConnection.getInputStream());
            // 리턴되어 오는 문자열을 읽어서 JSON형태로 파싱을 해야
            // 여기서 getStringFromInput은 미리 정해진 메서드가 아니고 위의 소스코드 처럼
            // InputStream 을 String으로 변환하기 위해서 여기서 지정된 코드들
    ...
    // JSON을 파싱해서, main.temp 값을 읽은후 Weather class에 넣어서 리턴
    private Weather parseJSON(JSONObject json) throws JSONException {
        Weather w = new Weather();
        // getJSONObject(“main”)을 이용하여 “main” json 문서를 얻고
        // “temp”의 값을 읽어서 Weather 객체에 세팅
